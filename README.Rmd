...
We have prepared the functionality in this package to operate on rasterized data from either the [raster](https://cran.r-project.org/package=raster) package or it's successor the [terra](https://cran.r-project.org/package=terra) package. Rather than using S3 class dispatch, handling the various inputs is done by simply testing that the input(s) inherit form either `BasicRaster` (raste package) or `SpatRaster` (terra package).
